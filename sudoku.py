...
def solve(grid):
    ...
    if grid is False:
        return False
    # If the length of all values are 1 -> puzzle solved!
    if all(len(grid[cell]) == 1 for cell in cells):
        return grid
    # The value with the lowest amount of possibilities is chosen
    number, cell = min((len(grid[s]), s) for s in cells if len(grid[s]) > 1)
